...
Mod para **Project Zomboid Build 42** que adiciona um atalho de teclado para disparar imediatamente o evento de helicóptero vanilla do jogo. Pressione **F7** (configurável) e o helicóptero aparece — com suporte a chance aleatória, confirmação por duplo clique e som de rádio militar.
...
4. **Chance** — sorteio baseado no valor configurado (padrão: 100%).
...
- **`testHelicopter()`**: função Java nativa, sem retorno nem evento de fim (`OnHelicopterEnd` não existe). Não há proteção interna contra disparos duplos — use a opção de confirmação por duplo clique se necessário.
...
└── Contents/mods/Pz-Helicopter-Events-Spawn/42/
    ...
    └── media/
        ...
        └── lua/
            ...
            └── client/
                └── Pz-Helicopter-Events-Spawn/
                    └── Main.lua            — lógica principal, doTrigger(), handler de teclado
```
